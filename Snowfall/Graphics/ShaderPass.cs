...
namespace Snowfall.Graphics
{
    public struct ShaderPass
    {
        ...
        {
            int index = GL.GetUniformLocation(program.ProgramId, name);
            if (index < 0) { Snowfall.Log("Attempt to use undefined uniform variable"); throw new Exception("Uniform variable not found!"); }
        ...
        public void SetUniform(string name, Texture2D value)
        {
            ...
            commands.Add(() => { GL.Uniform1(index, unitCounter);
